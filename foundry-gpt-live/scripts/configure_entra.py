...
PASSWORD_LABEL = "foundry-gpt-live-auth"
SESSION_SECRET_LABEL = "foundry-gpt-live-session"
MAX_RETRIES = 3
...
def _update_container_app(
    ...
    *,
    ...
    for _ in range(MAX_RETRIES):
        ...
        client_id = next(
            (item.get("value") for item in containers[0].get("env", []) or []
             if isinstance(item, dict) and item.get("name") == "AZURE_CLIENT_ID"),
            None,
        )
        if client_id != identity_client_id:
            raise RuntimeError("container app managed identity does not match the expected identity")
        ...
        by_name.update(
            {
                ...
                "BOOTSTRAP_LOCKED": {"name": "BOOTSTRAP_LOCKED", "value": "false"},
                "ALLOWED_USERS": {"name": "ALLOWED_USERS", "value": "tenant:*"},
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (
        ...
        raise RuntimeError("origin must be the generated Container Apps HTTPS origin")
