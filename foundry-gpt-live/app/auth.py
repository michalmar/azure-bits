...
SESSION_COOKIE_NAME = "__Host-gpt-live-session"
STATE_COOKIE_NAME = "__Host-gpt-live-login"
...
def load_auth_config_from_env() -> AuthConfig:
    ...
    allowed_users = tuple(
        value.strip().lower()
        for value in required["ALLOWED_USERS"].split(",")
        if value.strip()
    )
    if not allowed_users:
        raise RuntimeError("ALLOWED_USERS must explicitly allow at least one Entra identity")
...
class OIDCAuthManager:
    def __init__(self, config: AuthConfig, http_client: httpx.AsyncClient | None = None) -> None:
        ...
        self._session_serializer = URLSafeTimedSerializer(
            ...
            salt="foundry-gpt-live.session",
        )
        self._state_serializer = URLSafeTimedSerializer(
            config.session_secret or "",
            salt="foundry-gpt-live.state",
        )
    ...
    def _allowed(self, identity: dict[str, Any]) -> bool:
        if identity.get("tid") != self.config.tenant_id:
            return False
        oid = identity.get("oid")
        if not isinstance(oid, str) or not oid:
            return False
        return bool(
            {"tenant:*", f"entra:{oid.lower()}"}.intersection(self.config.allowed_users)
        )
    ...
    async def _validate_id_token(self, id_token: str, nonce: str) -> dict[str, Any]:
        ...
        try:
            claims = jwt.decode(
                ...
                options={
                    "require": ["exp", "iat", "nonce", "aud", "iss", "sub", "tid", "oid"],
                },
            )
    ...
    async def exchange_code(self, code: str, state_cookie: str, state_query: str) -> dict[str, Any]:
        ...
        if not self._allowed(claims):
            raise PermissionError("This identity is not allowed to use the demo.")
        return {
            "session": {
                "sub": claims.get("sub"),
                "tid": claims.get("tid"),
    ...
    def load_session(self, value: str | None) -> dict[str, Any] | None:
        if not value:
            return None
        identity = self._unsign_session(value)
        return identity if identity and self._allowed(identity) else None
